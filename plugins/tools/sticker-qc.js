const { sticker } = require('../../lib/sticker.js');
const axios = require('axios');


const handler = async (m, { conn, args, usedPrefix, command }) => {
    let text;
    if (args.length >= 1) {
        text = args.slice(0).join(" ");
    } else if (m.quoted && m.quoted.text) {
        text = m.quoted.text;
    } else {
        throw "Teksnya mana?\n\nContoh : .qc i love you";
    }

    if (!text) return m.reply('Teksnya mana?');
 //   if (text.length > 10000) return m.reply('Maximal 10000 karakter!');

    let pp;
    try {
        pp = await conn.profilePictureUrl(m.quoted ? m.quoted.sender : m.sender, 'image');
    } catch (error) {
        pp = 'https://i.ibb.co/3Fh9V6p/avatar-contact.png';
    }

    const obj = {
        "type": "quote",
        "format": "png",
        "backgroundColor": "#273443",
        "width": 512,
        "height": 768,
        "scale": 2,
        "messages": [{
            "entities": [],
            "avatar": true,
            "from": {
                "id": 1,
                "name": m.quoted ? db.data.users[m.quoted.sender].name : m.name,
                "photo": {
                    "url": "https://cors.caliph.my.id/" + pp
                }
            },
            "text": text,
            "replyMessage": {}
        }]
    };

    try {
        const response = await axios.post('https://bot.lyo.su/quote/generate', obj, {
            headers: {
                'Content-Type': 'application/json'
            }
        });

        const buffer = Buffer.from(response.data.result.image, 'base64');
        const stiker = await sticker(buffer, false, global.packname, global.author);

        if (stiker) return conn.sendFile(m.chat, stiker, 'Quotly.webp', '', m);
    } catch (error) {
        console.error('Error generating quote sticker:', error);
        return m.reply('Terjadi kesalahan saat membuat stiker kutipan.');
    }
};

handler.help = ['qc'];
handler.tags = ['sticker'];
handler.command = /^(qc)$/i;

module.exports = handler;