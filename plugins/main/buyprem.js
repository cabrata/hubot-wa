...
async function createQrisCanvas(qrString, amount, orderId) {
    ...
    ctx.fillText('— Wangsheng Funeral Parlor VIP —', width / 2, 108)
    ctx.restore()
...
// ════════════════════════════════════════════════════════════════════════════
async function createSuccessCanvas(amount, userName, packageName) {
    const width = 620
    const height = 480
    const canvas = createCanvas(width, height)
    const ctx = canvas.getContext('2d')
    ...
    ctx.fillText('✦ PEMBELIAN BERHASIL ✦', width / 2, 260)
    ctx.shadowBlur = 0
    ctx.restore()
    ...
    ctx.fillText(`Premium ${packageName}`, width / 2, 310)
    ctx.shadowBlur = 0
    ctx.restore()
    ...
    ctx.fillText('"Status Premium kamu telah diaktifkan otomatis."', width / 2, 393)
    ctx.restore()
    ...
    ctx.font = 'italic 12px serif'
    ctx.fillText('— Wangsheng Funeral Parlor VIP —', width / 2, 465)
    ctx.restore()
...
module.exports = {
    name: 'buyprem',
    command: ['buyprem', 'premium'],
    category: 'main',
    desc: 'Beli premium bot otomatis via QRIS',

    async handler({ m, conn, args, usedPrefix, command }) {
        if (!process.env.PAKASIR_PROJECT || !process.env.PAKASIR_KEY) {
            return m.reply(`❌ Fitur pembelian otomatis belum dikonfigurasi.\n\n_Note untuk Owner: Silakan isi PAKASIR_PROJECT dan PAKASIR_KEY di file .env,_ \natau hubungi owner untuk pembelian manual.`)
        }

        let packages = {
            '1': { name: '1 Bulan', price: 20000, days: 30 },
            '3': { name: '3 Bulan', price: 30000, days: 90 }
        }

        let plan = args[0]
        if (!plan || !packages[plan]) {
            let info = `👑 *PREMIUM BOT* 👑\n\nKeuntungan Premium:\n- Limit tidak terbatas\n- melewati akses Anti-spam & bot busy\n- Bisa custom watermark sticker (${usedPrefix}setwm)\n- Akses fitur premium seperti Spotify downloader\n- Akses fitur enhancer photo\n\n*Pilihan Paket Premium:*\n`
            info += `1. Rp 20.000 / 1 Bulan\n`
            info += `3. Rp 30.000 / 3 Bulan\n\n`
            info += `*Cara Beli:*\nKetik *${usedPrefix}${command} <nomor paket>*\nContoh: *${usedPrefix}${command} 1* (untuk beli paket 1 Bulan)`
            return m.reply(info)
        }

        let selectedPackage = packages[plan]
        let amount = selectedPackage.price

        let order_id = `PREMIUM-${m.sender.split('@')[0]}-${Date.now()}`
        await m.reply(`⏳ Sedang membuat Invoice Premium ${selectedPackage.name} via QRIS...`)
        ...
        try {
            ...
            if (!res.ok || !json.payment || !json.payment.payment_number) {
                ...
                return m.reply('❌ Gagal membuat transaksi. Pastikan PAKASIR_PROJECT dan PAKASIR_KEY sudah benar.')
            }
            ...
            let caption = `╭─「 *👑 INVOICE PREMIUM* 」
│ 🆔 *Order ID:* ${order_id}
│ 📦 *Paket:* Premium ${selectedPackage.name}
│ 💰 *Harga:* Rp ${amount.toLocaleString('id-ID')}
│ 💸 *Total (Inc. Fee):* Rp ${totalPayment.toLocaleString('id-ID')}
│ 🏦 *Metode:* QRIS Payment
...
_👑 Status otomatis dicek tiap 10 detik. Jika berhasil, kamu otomatis menjadi User Premium._`
            ...
            let interval = setInterval(async () => {
                ...
                try {
                    ...
                    if (statusJson?.transaction?.status === 'completed') {
                        ...
                        let newTime = (isPremium ? Number(user.premiumTime) : now) + (selectedPackage.days * 86400000)
                        ...
                        let completeMsg = `🎉 *PEMBELIAN BERHASIL!* 🎉\n\nSelamat *@${m.sender.split('@')[0]}*, kamu sekarang adalah User Premium!\n\nMasa Aktif: Bertambah ${selectedPackage.days} Hari\n\nKamu sekarang dapat mencoba fitur *${usedPrefix}setwm* untuk watermark stiker custom.`
                        let successBuffer = await createSuccessCanvas(amount, m.pushName || `@${m.sender.split('@')[0]}`, selectedPackage.name)
            ...
            m.reply('❌ Terjadi kesalahan sistem saat memproses pemesanan tiket.')
        }
    }
}
