// 📁 gpt-5-nano.js — sistem prompt + image_input + transkrip audio + reply support
const fs = require('fs');
...
`.trim();
  ...
  const input = {
    ...
    temperature: 1,
    top_p: 1,
    max_completion_tokens: 1024, // Disesuaikan untuk model 'nano' yang lebih ringkas
    frequency_penalty: 0,
    presence_penalty: 0
  };
    ...
    for await (const event of replicate.stream("openai/gpt-5-nano", { input })) {
      result += event.toString();
       ...
       if (e.message.includes('limit reached')) {
          ...
          return '[❌] Gagal mendapatkan respons dari GPT-5 nano\n' + e.message; 
       }
  }
  ...
  return result.trim() || '[❌] Tidak ada respons dari GPT-5 nano';
};
