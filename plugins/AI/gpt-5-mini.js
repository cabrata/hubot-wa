// 📁 gpt-5-mini.js — sistem prompt + image_input + transkrip audio + reply support
const fs = require('fs');
...
module.exports = async ({ teks, media, mimetype, audioTranscription, quoted, sender, profileData }) => {
  ...
  const maxHistoryLength = 10; // Batas maksimum 10 giliran (5 user, 5 assistant)
  ...
  // 2. Pembentukan System Prompt dengan Data User
...
`.trim();
    ...
    for await (const event of replicate.stream("openai/gpt-5-mini", { input })) {
      result += event.toString();
       ...
       if (e.message.includes('limit reached')) {
          ...
          return '[❌] Gagal mendapatkan respons dari GPT-5 mini\n' + e.message; 
       }
  }
  ...
  return result.trim() || '[❌] Tidak ada respons dari GPT-5 mini';
};
