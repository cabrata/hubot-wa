...
`.trim() 
  ...
  // 🟡 Tentukan file sistem prompt berdasarkan role
  ...
  // Role prioritas: Owner > Mods > Premium > Biasa
  if (isROwner) {
    maxHist = 14; maxToken = 4096; temp = 1; topP = 1;
  } else if (isMods) {
    maxHist = 6; maxToken = 1024; temp = 0.6; topP = 0.6;
  } else if (isPrems) {
  ...
  try {
    ...
    if (e.message.includes('limit reached')) {
      ...
      return '[❌] Gagal mendapatkan respons dari GPT-4.1 nano\n' + e.message;
    }
  }
  ...
  return result.trim() || '[❌] Tidak ada respons dari GPT-4.1-nano';
};
