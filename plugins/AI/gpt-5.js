// 📁 gpt-5.js — sistem prompt + image_input + transkrip audio + reply support
const fs = require('fs');
...
`.trim();
    ...
    for await (const event of replicate.stream("openai/gpt-5", { input })) {
      result += event.toString();
       ...
       if (e.message.includes('limit reached')) {
          ...
          return '[❌] Gagal mendapatkan respons dari GPT-5\n' + e.message; 
       }
  }
  ...
  return result.trim() || '[❌] Tidak ada respons dari GPT-5';
};
