...
const roleAmount = (from, data) => {
    ...
    // werewolf scale
    let werewolf = Math.max(1, Math.floor(n / 4));
    if (werewolf > Math.floor(n / 3)) werewolf = Math.floor(n / 3);

    // progressive roles
    let seer = n >= 6 ? 1 : 0;
    let guardian = n >= 6 ? 1 : 0;
    let sheriff = n >= 7 ? 1 : 0;
    let sorcerer = n >= 10 ? 1 : 0;
    let witch = n >= 11 ? 1 : 0;
    let hunter = n >= 13 ? 1 : 0;
    let cupid = n >= 12 ? 1 : 0;
    let mayor = n >= 13 ? 1 : 0;
    let joker = n >= 12 ? 1 : 0;   // <<< Joker lebih awal
    let thief = n >= 13 ? 1 : 0;   // <<< Thief muncul di 15+

    // total assigned roles
    let assigned = werewolf + seer + guardian + sheriff + sorcerer + witch + hunter + cupid + mayor + thief + joker;
...
const voteKill = (from, data) => {
    ...
    if (room.player[0].role === 'hunter') {
        let hunterNum = room.player[0].number;
    ...
    if (room.lovers && room.lovers.length === 2) {
        let votedNum = room.player[0].number;
...
const killww = (from, id, data) => {
    ...
    for (let j = 0; j < room.dead.length; j++) {
        ...
        if (!idd) return false;
        if (room.player[idd.index].effect.includes("guardian")) continue; // guardian saved
        room.player[idd.index].isdead = true;
        global.db.data.chats[from].blacklistUsers[room.player[idd.index].id] = true;
        // if hunter died, handle as in voteKill: kill chosen or random
        ...
        // lovers: if a lover dies, other lover dies too
        if (room.lovers && room.lovers.length === 2) {
...
async function malam(conn, x, data) {
    ...
    if (hasil_vote === 0) {
        ...
        // Normal flow: announce role and kill
